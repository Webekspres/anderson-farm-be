...
describe('CoopEquipment API', function () {
    ...
    it('can list equipments for a coop with equipment_type relation', function () {
        ...
        CoopEquipment::factory()->count(2)->create(['coop_id' => $coop->id, 'equipment_type_id' => $equipmentType->id]);
    ...
    it('can create a new equipment', function () {
        ...
        $payload = ['equipment_type_id' => $equipmentType->id, 'unit_code' => 'UNIT-1', 'installed_at' => now()->toISOString()];
        ...
        $this->assertDatabaseHas('coop_equipments', ['unit_code' => 'UNIT-1', 'coop_id' => $coop->id]);
    });

    it('fails with duplicate unit_code in same coop', function () {
        $coop = Coop::factory()->create();
        $equipmentType = EquipmentType::factory()->create();
        CoopEquipment::factory()->create(['coop_id' => $coop->id, 'equipment_type_id' => $equipmentType->id, 'unit_code' => 'DUP-1']);

        $payload = ['equipment_type_id' => $equipmentType->id, 'unit_code' => 'DUP-1'];
        $response = $this->postJson("/api/v1/coops/{$coop->id}/equipments", $payload);
    ...
    it('allows same unit_code in different coop', function () {
        ...
        CoopEquipment::factory()->create(['coop_id' => $coop1->id, 'equipment_type_id' => $equipmentType->id, 'unit_code' => 'SAME-1']);
        $payload = ['equipment_type_id' => $equipmentType->id, 'unit_code' => 'SAME-1'];
    ...
    it('delete cascades coop_form_assignments', function () {
        $coop = Coop::factory()->create();
        $equipmentType = EquipmentType::factory()->create();
        $equipment = CoopEquipment::factory()->create(['coop_id' => $coop->id, 'equipment_type_id' => $equipmentType->id]);
    ...
    it('returns 404 when deleting equipment of another coop', function () {
        ...
        $coop2 = Coop::factory()->create();
        $equipmentType = EquipmentType::factory()->create();
        $equipment = CoopEquipment::factory()->create(['coop_id' => $coop1->id, 'equipment_type_id' => $equipmentType->id]);
